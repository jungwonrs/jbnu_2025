...
def wm_metrics(pred: np.ndarray,
               gt  : np.ndarray):
    """
    pred : (2,)      ─ extract()가 반환한 예측 비트
    gt   : (2,H,W) 또는 (2,) ─ 정답 비트 맵/벡터
    반환값: acc, ber, nc
    """
    # gt가 맵(3-차원)이라면 → 공간 평균으로 (2,) 벡터화
    if gt.ndim == 3:
    ...
    acc = (pred == gt).mean()
    ber = (pred != gt).mean()
    nc  = np.logical_xor(pred, gt).sum() / gt.size
    return acc, ber, nc
...
# ───────── B+C 전용 평가 함수 ──────────────────────────────
def evaluate_BC(tag, rec_img, coeff_tensor,
                net_lh, net_hl,
                z_base_lh, z_base_hl):
    ...
    def _print(line): print(f"[{tag:5s} | {line}")

    pred = extract_two_nets(coeff_tensor, net_lh, net_hl,
                            z_base_lh, z_base_hl)
    acc, ber, nc = wm_metrics(pred, wm_bits_rand)
    _print(f"clean ]  PSNR {p0:6.2f} SSIM {s0:.4f} | "
           f"ACC {acc*100:6.2f}% BER {ber*100:5.2f}% NC {nc:.3f}")

    # JPEG
    for q in JPEG_Q:
        atk_u8 = jpeg(base_u8, q)
        atk_f  = atk_u8.astype(np.float32)/255.
        atk_c  = spatial2coeff(atk_f, "2")
        pred   = extract_two_nets(atk_c, net_lh, net_hl,
                                  z_base_lh, z_base_hl)
        acc, ber, nc = wm_metrics(pred, wm_bits_rand)
        p, s = psnr(gray, atk_u8, data_range=255), \
               ssim(gray, atk_u8, data_range=255, win_size=7)
        _print(f"JPEG{q:02d}] PSNR {p:6.2f} SSIM {s:.4f} | "
               f"ACC {acc*100:6.2f}% BER {ber*100:5.2f}% NC {nc:.3f}")
    ...
    for sg in GB_SIG:
        ...
        p, s = psnr(gray, atk_u8, data_range=255), \
               ssim(gray, atk_u8, data_range=255, win_size=7)
        _print(f"Blur{sg}]  PSNR {p:6.2f} SSIM {s:.4f} | "
               f"ACC {acc*100:6.2f}% BER {ber*100:5.2f}% NC {nc:.3f}")

# ───────── 평가 함수 ──────────────────────────────────────────
def evaluate(tag, rec_img, coeff_tensor,
             model, two_ch, z_base):
    base_u8 = to_u8(rec_img)
    p0, s0  = psnr(gray, base_u8, data_range=255), \
              ssim(gray, base_u8, data_range=255, win_size=7)
    ...
    for q in JPEG_Q:
        ...
        pred    = extract(atk_coef, model, two_ch, z_base)
        acc,ber,nc = wm_metrics(pred, wm_bits_rand)
        p,s = psnr(gray, atk_u8, data_range=255), \
              ssim(gray, atk_u8, data_range=255, win_size=7)
        _print(f"JPEG{q:02d}] PSNR {p:6.2f} SSIM {s:.4f} | "
