...
def subnet(din, dout):
    print("din>>>>>>>>", din)
    hidden_size = min(512, int(din * 0.25))
...
LH_stego = out[0, :LH_size].detach().view(LH.shape).numpy()
HL_stego = out[0, LH_size:LH_size+HL_size].detach().view(HL.shape).numpy()
...
def reshape_square_with_padding(bits):
    length = len(bits)
    side = ceil(np.sqrt(length))  
    padded = np.pad(bits, (0, side * side - length), constant_values=0)
    return padded.reshape(side, side) * 255

wm_rec = reshape_square_with_padding(wm_all_bits)

print("Recovered watermark:", decode_watermark(wm_all_bits))
