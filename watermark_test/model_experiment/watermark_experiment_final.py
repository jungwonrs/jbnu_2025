...
if len(sys.argv) < 2:
    raise RuntimeError("Usage: python watermark_experiment_final.py <tag>")

tag = sys.argv[1]
ckpt_dir = os.path.join(MODEL_DIR, tag)

pathA = os.path.join(ckpt_dir, "both", "inn_both.pth")   # A 모델
pathB = os.path.join(ckpt_dir, "lh", "inn_lh.pth")     # B-LH 모델
pathC = os.path.join(ckpt_dir, "hl", "inn_hl.pth")     # C-HL 모델
pathF = os.path.join(ckpt_dir, "full", "inn_full.pth")   # Full 모델
...
# 입력 DWT 계수
img_path = r"C:\Users\seo\Desktop\watermark_test\test_image.jpg"
gray = cv2.cvtColor(cv2.resize(cv2.imread(img_path), (256, 256)),
                    cv2.COLOR_BGR2GRAY)
...
if combine_BC:
    ...
    print(f"{'B+C':<8}▶ ACC {ACC*100:6.2f}% | BER {BER*100:5.2f}% "f"| NC {NC:.3f} | IF-RMSE -") 
